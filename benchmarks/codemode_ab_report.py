...
def paired(rows, key):
    ...
    return [(k, v["steered"] - v["control"], v["control"], v["steered"])
            for k, v in by.items() if "steered" in v and "control" in v]
...
def main() -> int:
    ...
    for arm in ("control", "steered"):
        ...
        for f, label, unit in (("cost_usd", "cost", "$"), ("num_turns", "turns", ""),
                               ("tool_calls", "tool calls", ""), ("bash_calls", "bash calls", ""),
                               ("compound_bash", "compound bash", ""),
                               ("fetched_chars", "bytes fetched", ""), ("wall_s", "wall", "s")):
            vals = [r.get(f) or 0 for r in a]
            print(f"   {label:16s} mean {unit}{statistics.mean(vals):>10.4f}   "
                  f"median {unit}{statistics.median(vals):>10.4f}   total {unit}{sum(vals):>12.2f}")

    print("\n=== PAIRED (steered - control), per task ===")
    for f, label in (("cost_usd", "cost $"), ("num_turns", "turns"), ("tool_calls", "tool calls"),
                     ("fetched_chars", "bytes fetched"), ("compound_bash", "compound bash")):
        d = paired(rows, f)
        if not d:
            continue
        ...
        line = (f"  {label:16s} mean Δ {mean:+12.4f}   total Δ {sum(deltas):+12.4f} "
    ...
    for (task, rep), delta, c, s in sorted(d):
